...
namespace UpgrowAdminPanel.Controllers
{
    public class MasterController : BaseController
    {
        ...
        // Remove Icon and Description. Add Sequence No

        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master API Category List", MenuName = "Master_ApiCategory")]
        public IActionResult MasterViewApiCategory() => View();

        [HttpPost]
        public Task<IActionResult> GetApiCategory()
        => GetPagedDataAsync<MasterApiCategoryDto, MasterApiCategoryCommand>(_apiCategoryService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["categoryName"] = dto.CategoryName,
            ["sequenceno"] = dto.SequenceNo,
            ["icon"] = dto.Icon,
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleApiCategory), "Api Category")
        });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master API Category", MenuName = "Master_ApiCategory")]
        public IActionResult MasterAddApiCategory()
        {
            return View("MasterAddApiCategory", new MasterApiCategoryCommand
            {
                IsActive = true
            });
        }
        ...
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master API Category", MenuName = "Master_ApiCategory")]
        public Task<IActionResult> MasterEditApiCategory(string? uuid)
            => EditMasterAsync(
                uuid,
                _apiCategoryService,
                () => new MasterApiCategoryCommand { IsActive = true },
                dto => new MasterApiCategoryCommand
                {
                    UUID = dto.UUID,
                    CategoryName = dto.CategoryName,
                    SequenceNo = dto.SequenceNo,
                ...
                "MasterAddApiCategory",
                nameof(MasterViewApiCategory));

        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master API Category", MenuName = "Master_ApiCategory")]
        public Task<IActionResult> MasterAddApiCategory(MasterApiCategoryCommand command)
            => SaveMasterAsync(
                command,
                _apiCategoryService,
                "Api Category",
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master API Category Status", MenuName = "Master_ApiCategory")]
        public Task<IActionResult> ToggleApiCategory(string uuid)
            => ToggleActiveAsync<MasterApiCategoryDto, MasterApiCategoryCommand>(uuid, _apiCategoryService, "Api Category");
       ...
       => GetPagedDataAsync<MasterEmailCredentialDto, MasterEmailCredentialCommand>(_emailCredentialService, dto => new Dictionary<string, object>
       {
        ...
        // Updated by Krishna (23-04-2026)
        // Change label from Display Order to Sequence No 

        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master API List", MenuName = "Master_API")]
        public IActionResult MasterViewApi() => View();

        [HttpPost]
        public Task<IActionResult> GetApi()
            => GetPagedDataAsync<MasterApiDto, MasterApiCommand>(_apiService, dto => new Dictionary<string, object>
            {
                ["uuid"] = dto.UUID,
                ["apiname"] = dto.ApiName,
                ["code"] = dto.Code,
                ["category"] = dto.ApiCategoryUUID,  // Now shows the category name (from the join)
                ["description"] = dto.ShortDescription ?? "",
                ["displayorder"] = dto.DisplayOrder,
                ["consentbased"] = dto.IsConsentBased
                    ? "<span class='badge badge-outline-success'>Yes</span>"
                    : "<span class='badge badge-outline-danger'>No</span>",
                ["reminderrequired"] = dto.IsReminderRequired
                    ? "<span class='badge badge-outline-success'>Yes</span>"
                    : "<span class='badge badge-outline-danger'>No</span>",
                ["isproviderswitchable"] = dto.IsProviderSwitchable
                    ? "<span class='badge badge-outline-success'>Yes</span>"
                    : "<span class='badge badge-outline-danger'>No</span>",
                ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleApi), "Api")
            });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master API", MenuName = "Master_API")]
        public async Task<IActionResult> MasterAddApi()
        {
            var vm = new MasterApiVM
            {
                ApiCategoryList = await LoadDropdownAsync(_apiCategoryService, x => x.CategoryName),
                ComponentList = await LoadDropdownAsync(_componentService, x => x.Name)
            };
            int nextDisplayOrder = 1; // Default to 1

            try
            {
                // Get all active APIs and find the max DisplayOrder
                var allApis = await _apiService.GetAllActiveAsync();
                if (allApis != null && allApis.Count > 0)
                {
                    int maxOrder = allApis.Max(x => x.DisplayOrder);
            ...
            vm.Api = new MasterApiCommand
            {
                UUID = null,
                IsActive = true,
                DisplayOrder = nextDisplayOrder  // Show next number (e.g., 1, 2, 3, etc.)
            };
            var providers = await _apiProviderService.GetAllActiveAsync();

            vm.ProviderMappings = providers.Select(x => new ProviderRowVM
            {
                ProviderUUID = x.UUID,
            ...
            return View("MasterAddApi", vm);
        }

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master API", MenuName = "Master_API")]
        public async Task<IActionResult> MasterEditApi(string? uuid)
        {
            var vm = new MasterApiVM
            {
                ApiCategoryList = await LoadDropdownAsync(_apiCategoryService, x => x.CategoryName),
            ...
            if (string.IsNullOrEmpty(uuid))
                return RedirectToAction(nameof(MasterAddApi));

            var dto = await _apiService.GetByUuidAsync(uuid);

            if (dto == null)
            {
                SetErrorMessage("Record not found!");
                return RedirectToAction(nameof(MasterViewApi));
            }
            ...
            vm.Api = new MasterApiCommand
            {
                ...
                DisplayOrder = dto.DisplayOrder,
                IsConsentBased = dto.IsConsentBased,
                IsReminderRequired = dto.IsReminderRequired,
                IsProviderSwitchable = dto.IsProviderSwitchable,
                IsActive = dto.IsActive,
                IsMultipleEndPoint = dto.IsMultipleEndPoint
            };


            var endpoints = await _apiEndpointService.GetByApiAsync(uuid);

            if (endpoints != null && endpoints.Count > 0)
            {
                // First endpoint (sequence 1)
                var firstEndpoint = endpoints.FirstOrDefault(x => x.Sequence == 1);
                if (firstEndpoint != null)
                {
                    vm.ApiEndpoint = new ApiEndpointCommand
                    {
                        UUID = firstEndpoint.UUID,
                        ApiUUID = firstEndpoint.ApiUUID,
                        EndpointUrl = firstEndpoint.EndpointUrl,
                        HttpMethod = firstEndpoint.HttpMethod,
                        IsActive = firstEndpoint.IsActive
                    };
                }

                // ✅ Second endpoint (sequence 2) if exists
                var secondEndpoint = endpoints.FirstOrDefault(x => x.Sequence == 2);
                if (secondEndpoint != null)
                {
                    vm.SecondApiEndpoint = new ApiEndpointCommand
                    {
                        UUID = secondEndpoint.UUID,
                        ApiUUID = secondEndpoint.ApiUUID,
                        EndpointUrl = secondEndpoint.EndpointUrl,
                        HttpMethod = secondEndpoint.HttpMethod,
                        IsActive = secondEndpoint.IsActive
                    };
                }
            }

            var components = await _componentMappingService.GetByApiUuidAsync(uuid);

            if (components != null && components.Count > 0)
            {
                // First endpoint (sequence 1)
                var firstComponent = components.FirstOrDefault(x => x.Sequence == 1);
                if (firstComponent != null)
                {
                    vm.SelectedComponentUUID = firstComponent.ComponentUUID;
                }

                // ✅ Second endpoint (sequence 2) if exists
                var secondComponent = components.FirstOrDefault(x => x.Sequence == 2);
                if (secondComponent != null)
                {
                    vm.SelectedSecondComponentUUID = secondComponent.ComponentUUID;
                }
            }



            var componentMapping = await _componentMappingService.GetByApiUuidAsync(dto.UUID);

            var selectedComponent = componentMapping?.FirstOrDefault();

            vm.SelectedComponentUUID = selectedComponent?.ComponentUUID;
            var allProviders = await _apiProviderService.GetAllActiveAsync();

            var existingProviderApis = await _providerApisService.GetByApiUuidAsync(dto.UUID);
            var existingMappings = await _apiProviderMappingService.GetByApiUuidAsync(dto.UUID);

            vm.ProviderMappings = allProviders.Select(p =>
            {
                var support = existingProviderApis.FirstOrDefault(x => x.ProviderUUID == p.UUID);
                var mapping = existingMappings.FirstOrDefault(x => x.ProviderUUID == p.UUID);

                return new ProviderRowVM
                {
                    ProviderUUID = p.UUID,
                    ProviderName = p.ProviderName,
                    SupportsApi = support != null,
        ...
        [ActivityLog(
        ActivityType = "Click",
        Description = "Clicked On Add/Edit Master API",
        MenuName = "Master_API")]
        public async Task<IActionResult> MasterAddApi(MasterApiVM vm)
        {
            try
            {
                if (vm.Api.IsMultipleEndPoint == false)
                {
                    ModelState.Remove("SecondApiEndpoint.EndpointUrl");
                    ModelState.Remove("SecondApiEndpoint.HttpMethod");
                    ModelState.Remove("SelectedSecondComponentUUID");
                }
                ...
                var supportedProviders =
                    ...
                    .ToList()
                    ?? new List<ProviderRowVM>();

                if (vm.Api.IsProviderSwitchable != true)
                {
                    if (supportedProviders.Count > 1)
                        throw new Exception("Only one provider allowed when switch is OFF.");

                    if (supportedProviders.Any())
                    {
                        supportedProviders[0].IsActive = true;
                        supportedProviders[0].Priority = 1;
                    }
                }

                // Active providers AFTER rules
                var activeProviders =
                    supportedProviders
                    .Where(x => x.IsActive)
                    .ToList();
                ...
                if (vm.Api.IsProviderSwitchable == true)
                {
                    ...
                    if (activeProviders.Count > 3)
                        throw new Exception("Maximum 3 active providers allowed.");

                    if (activeProviders.Count < 2)
                        throw new Exception("atleast 2 active providers allowed.");

                    if (activeProviders.Any(x => x.Priority == null))
                        throw new Exception("Priority required for active providers.");
                    ...
                    if (activeProviders
                        ...
                        .Any(g => g.Count() > 1))
                    {
                        throw new Exception("Duplicate priority not allowed.");
                    }
                }

                var command = new SaveApiConfigurationCommand
                {
                    Api = vm.Api,
                    ApiEndpoint = vm.ApiEndpoint,
                    SecondApiEndpoint = vm.SecondApiEndpoint,
                    SelectedComponentUUID = vm.SelectedComponentUUID,
                    SelectedSecondComponentUUID = vm.SelectedSecondComponentUUID,

                    ProviderMappings =
                    vm.ProviderMappings
                    .Select(x => new ProviderConfigurationCommand
                    {
                        ProviderUUID = x.ProviderUUID,
                        SupportsApi = x.SupportsApi,
                        IsActive = x.IsActive,
                        Priority = x.Priority
                    })
                    .ToList()
                ...
                await _manageApiService.SaveApiConfigurationAsync(
                    command,
                    GetUserUUID(),
                    Utils.GetLocalIPAddress());

                SetSuccessMessage(
                    string.IsNullOrEmpty(vm.Api.UUID)
                    ? "API basic data added successfully!"
                    : "API basic data updated successfully!");
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master API Status", MenuName = "Master_API")]
        public Task<IActionResult> ToggleApi(string uuid)
            => ToggleActiveAsync<MasterApiDto, MasterApiCommand>(uuid, _apiService, "Api");

        #endregion

        #region Provider APIs
        // Updated by Krishna (23-04-2026)
        // Remove Step Code, Http Method and Endpoint URL
        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master Provider API List", MenuName = "Master_Provider_API")]
        public IActionResult MasterViewProviderApis() => View();

        [HttpPost]
        public Task<IActionResult> GetProviderApis()
            => GetPagedDataAsync<ProviderApisDto, ProviderApisCommand>(_providerApisService, dto => new Dictionary<string, object>
            {
                ["uuid"] = dto.UUID,
                ["provider"] = dto.ProviderUUID,
                ["api"] = dto.ApiUUID,
                ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleProviderApis), "Provider API")
            });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Provider API", MenuName = "Master_Provider_API")]
        public async Task<IActionResult> MasterAddProviderApis()
        {
            var vm = new ProviderApisVM
            {
                ApiList = await LoadDropdownAsync(_apiService, x => x.ApiName),
                ProviderList = await LoadDropdownAsync(_apiProviderService, x => x.ProviderName)
            };
            vm.ProviderApi = new ProviderApisCommand { IsActive = true };
            return View("MasterAddProviderApis", vm);
        }

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master Provider API", MenuName = "Master_Provider_API")]
        public async Task<IActionResult> MasterEditProviderApis(string? uuid)
        {
            var vm = new ProviderApisVM
            {
                ApiList = await LoadDropdownAsync(_apiService, x => x.ApiName),
            ...
            var dto = await _providerApisService.GetByUuidAsync(uuid);

            if (dto == null)
            {
                SetErrorMessage("Record not found!");
                return RedirectToAction(nameof(MasterViewProviderApis));
            }

            vm.ProviderApi = new ProviderApisCommand
            {
                UUID = dto.UUID,
                ApiUUID = dto.ApiUUID,
        ...
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Provider API", MenuName = "Master_Provider_API")]
        public async Task<IActionResult> MasterAddProviderApis(ProviderApisVM vm)
        {
            if (vm == null)
                return BadRequest();

            if (!TryValidateModel(vm.ProviderApi, nameof(vm.ProviderApi)))
            {
                vm.ApiList = await LoadDropdownAsync(_apiService, x => x.ApiName);
                vm.ProviderList = await LoadDropdownAsync(_apiProviderService, x => x.ProviderName);
                return View("MasterAddProviderApis", vm);
            }

            try
            {
                await _providerApisService.SaveAsync(vm.ProviderApi, GetUserUUID(), Utils.GetLocalIPAddress());

                SetSuccessMessage(string.IsNullOrEmpty(vm.ProviderApi.UUID)
                    ? "Provider API added successfully!"
                    : "Provider API updated successfully!");
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master Provider API Status", MenuName = "Master_Provider_API")]
        public Task<IActionResult> ToggleProviderApis(string uuid)
            => ToggleActiveAsync<ProviderApisDto, ProviderApisCommand>(uuid, _providerApisService, "Provider API");
        #endregion
    ...
    => GetPagedDataAsync<MasterBlogDto, MasterBlogCommand>(_blogService, dto => new Dictionary<string, object>
    {
        ...
        #region TrustedPartners

        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Trusted Partners List", MenuName = "Trusted_Partners")]
        public IActionResult ViewTrustedPartners() => View();

        [HttpPost]
        public Task<IActionResult> GetTrustedPartners()
        => GetPagedDataAsync<TrustedPartnersDto, TrustedPartnersCommand>(_trustedPartnersService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["name"] = dto.Name,
            ["icon"] = !string.IsNullOrEmpty(dto.IconImage)
                  ? $"<img src='{_domainResolverService.BuildAbsoluteUrl(dto.IconImage)}' alt='Partner Icon' style='width: 50px; height: 50px; object-fit: cover; border-radius: 4px;' />"
                  : "",
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleTrustedPartners), "Trusted Partners")
        });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Trusted Partner", MenuName = "Trusted_Partners")]
        public IActionResult AddTrustedPartners() => View("AddTrustedPartners", new TrustedPartnersCommand { IsActive = true });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Trusted Partner", MenuName = "Trusted_Partners")]
        public Task<IActionResult> EditTrustedPartners(string? uuid)
            => EditMasterAsync(
                uuid,
                _trustedPartnersService,
                () => new TrustedPartnersCommand { IsActive = true },
                dto => new TrustedPartnersCommand
                {
                    UUID = dto.UUID,
                    Name = dto.Name,
                    IconImage = dto.IconImage,
                ...
                "AddTrustedPartners",
                nameof(ViewTrustedPartners));

        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Trusted Partner", MenuName = "Trusted_Partners")]
        public async Task<IActionResult> AddTrustedPartners(TrustedPartnersCommand command)
        {
            if (!ModelState.IsValid)
            {

                return View("AddTrustedPartners", command);
            }
            ...
            try
            {
                ...
                await _trustedPartnersService.SaveAsync(command, GetUserUUID(), Utils.GetLocalIPAddress());

                SetSuccessMessage(string.IsNullOrEmpty(command.UUID)
                    ? "Trusted Partner added successfully!"
                    : "Trusted Partner updated successfully!");

                return RedirectToAction(nameof(ViewTrustedPartners));
            }
            catch (Exception ex)
            {
                SetErrorMessage(ex.Message);
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Trusted Partner Status", MenuName = "Trusted_Partners")]
        public Task<IActionResult> ToggleTrustedPartners(string uuid)
            => ToggleActiveAsync<TrustedPartnersDto, TrustedPartnersCommand>(uuid, _trustedPartnersService, "Trusted Partners");
        ...
        #region DosDonts
        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master DosDonts List", MenuName = "Master_DosDonts")]
        public IActionResult MasterViewDosDonts() => View();

        [HttpPost]
        public Task<IActionResult> GetDosDonts()
        => GetPagedDataAsync<MasterDosDontsDto, MasterDosDontsCommand>(_masterDosDontsService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["message"] = dto.Message,
            ["type"] = dto.IsDos ? "DO'S" : "DON'TS",
            ["sequenceno"] = dto.SequenceNo,
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleDosDonts), "Dos/Donts")
        });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master DosDonts", MenuName = "Master_DosDonts")]
        public IActionResult MasterAddDosDonts() => View("MasterAddDosDonts", new MasterDosDontsCommand { IsActive = true });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master DosDonts ", MenuName = "Master_DosDonts")]
        public Task<IActionResult> MasterEditDosDonts(string? uuid)
            => EditMasterAsync(
                uuid,
                _masterDosDontsService,
                () => new MasterDosDontsCommand { IsActive = true },
                dto => new MasterDosDontsCommand
                {
                    UUID = dto.UUID,
                    Message = dto.Message,
                    IsDos = dto.IsDos,
                ...
                "MasterAddDosDonts",
                nameof(MasterViewDosDonts));

        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master DosDonts ", MenuName = "Master_DosDonts")]
        public Task<IActionResult> MasterAddDosDonts(MasterDosDontsCommand command)
            => SaveMasterAsync(
                command,
                _masterDosDontsService,
                "DOS/DONTS",
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toogle On Master DosDonts Status ", MenuName = "Master_DosDonts")]
        public Task<IActionResult> ToggleDosDonts(string uuid)
            => ToggleActiveAsync<MasterDosDontsDto, MasterDosDontsCommand>(uuid, _masterDosDontsService, "DOS/DONTS");
        #endregion
        ...
        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master Programming Language List", MenuName = "Master_ProgrammingLanguage")]
        public IActionResult MasterViewProgrammingLanguage() => View();

        [HttpPost]

        public Task<IActionResult> GetProgrammingLanguages()
        => GetPagedDataAsync<MasterProgrammingLanguageDto, MasterProgrammingLanguageCommand>(_masterProgrammingLanguageService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["name"] = dto.Title,
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleProgrammingLanguage), "Programming Language")
        });
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Programming Language", MenuName = "Master_ProgrammingLanguage")]
        public IActionResult MasterAddProgrammingLanguage()
        {
            return View("MasterAddProgrammingLanguage", new MasterProgrammingLanguageCommand
            {
                IsActive = true
            });
        }
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master Programming Language", MenuName = "Master_ProgrammingLanguage")]
        public Task<IActionResult> MasterEditProgrammingLanguage(string? uuid)
            => EditMasterAsync(
                uuid,
                _masterProgrammingLanguageService,
                () => new MasterProgrammingLanguageCommand { IsActive = true },
                dto => new MasterProgrammingLanguageCommand
                {
                    UUID = dto.UUID,
                    Title = dto.Title,
                    IsActive = dto.IsActive
                },
                "MasterAddProgrammingLanguage",
                nameof(MasterViewProgrammingLanguage));

        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Programming Language", MenuName = "Master_ProgrammingLanguage")]
        public Task<IActionResult> MasterAddProgrammingLanguage(MasterProgrammingLanguageCommand command)
            => SaveMasterAsync(
                command,
                _masterProgrammingLanguageService,
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master Programming Language Status", MenuName = "Master_ProgrammingLanguage")]
        public Task<IActionResult> ToggleProgrammingLanguage(string uuid)
            => ToggleActiveAsync<MasterProgrammingLanguageDto, MasterProgrammingLanguageCommand>(uuid, _masterProgrammingLanguageService, "Programming Language");
        #endregion

        #region ApiInfoSection 


        private async Task BindapiDropdowns(ApiInfoSectionVM vm)
        {
        ...
        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Api Info Section List", MenuName = "ApiInfoSection")]
        public IActionResult ViewApiInfoSection() => View();

        [HttpPost]
        public Task<IActionResult> GetApiInfoSection()
        => GetPagedDataAsync<ApiInfoSectionDto, ApiInfoSectionCommand>(_apiInfoSectionService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["api"] = dto.ApiUUID,
            ["title"] = dto.Title,
            ...
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleApiInfoSection), "Api Info Section")
        });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Api Info Section", MenuName = "ApiInfoSection")]
        public async Task<IActionResult> AddApiInfoSection()
        {

            var vm = new ApiInfoSectionVM
            {
                ApiInfoSection = new ApiInfoSectionCommand { IsActive = true }
            };
        ...
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Api Info Section", MenuName = "ApiInfoSection")]
        public async Task<IActionResult> EditApiInfoSection(string? uuid)
        {
            var vm = new ApiInfoSectionVM
            {
                ApiInfoSection = new ApiInfoSectionCommand { IsActive = true }
            ...
            if (!string.IsNullOrWhiteSpace(uuid))
            {
                var dto = await _apiInfoSectionService.GetByUuidAsync(uuid);

                if (dto != null)
                {
                    vm.ApiInfoSection = new ApiInfoSectionCommand
                    {
                        UUID = dto.UUID,
                        ApiUUID = dto.ApiUUID,
                        Title = dto.Title,
        ...
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Api Info Section", MenuName = "ApiInfoSection")]
        public async Task<IActionResult> AddApiInfoSection(ApiInfoSectionVM vm)
        {
            ...
            try
            {
                ...
                await _apiInfoSectionService.SaveAsync(
                    vm.ApiInfoSection,
                    GetUserUUID(),
                    Utils.GetLocalIPAddress()
                );

                SetSuccessMessage(string.IsNullOrEmpty(vm.ApiInfoSection.UUID)
                    ? "Api endpoint added successfully!"
                    : "Api endpoint updated successfully!");

                return RedirectToAction(nameof(ViewApiInfoSection));
            }
            catch (Exception ex)
            {
                SetErrorMessage(ex.Message);
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master Api Info Section Status", MenuName = "ApiInfoSection")]
        public Task<IActionResult> ToggleApiInfoSection(string uuid)
            => ToggleActiveAsync<ApiInfoSectionDto, ApiInfoSectionCommand>(uuid, _apiInfoSectionService, "Api Info Section");
        ...
        #region Master Offer
        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master Offer List", MenuName = "Master_Offer")]
        public IActionResult MasterViewOffer() => View();

        [HttpPost]

        public Task<IActionResult> GetOffers()
        => GetPagedDataAsync<MasterOfferDto, MasterOfferCommand>(_offerService, dto => new Dictionary<string, object>
        {
            ["uuid"] = dto.UUID,
            ["name"] = dto.Name,
            ...
            ["validFrom"] = dto.ValidFrom?.ToString("dd-MM-yyyy"),
            ["validTo"] = dto.ValidTo?.ToString("dd-MM-yyyy"),
            ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleOffer), "Offer")
        });
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Offer", MenuName = "Master_Offer")]
        public IActionResult MasterAddOffer()
        {
            return View("MasterAddOffer", new MasterOfferCommand
            {
                IsActive = true
            });
        }
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master Offer", MenuName = "Master_Offer")]
        public Task<IActionResult> MasterEditOffer(string? uuid)
            => EditMasterAsync(
                uuid,
                _offerService,
                () => new MasterOfferCommand { IsActive = true },
                dto => new MasterOfferCommand
                {
                    UUID = dto.UUID,
                    Name = dto.Name,
                    OfferCode = dto.OfferCode,
                    ...
                    PerUserUsageCount = dto.PerUserUsageCount,
                    IsFirstTransactionOnly = dto.IsFirstTransactionOnly,
                    IsApplicableForAllUsers = dto.IsApplicableForAllUsers,
                    IsActive = dto.IsActive
                },
                "MasterAddOffer",
                nameof(MasterViewOffer));

        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Offer", MenuName = "Master_Offer")]
        public Task<IActionResult> MasterAddOffer(MasterOfferCommand command)
            => SaveMasterAsync(
                command,
                _offerService,
                "Offer",
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master Offer Status", MenuName = "Master_Offer")]
        public Task<IActionResult> ToggleOffer(string uuid)
            => ToggleActiveAsync<MasterOfferDto, MasterOfferCommand>(uuid, _offerService, "Offer");
        #endregion
        ...
        #region Website Industry

        [HttpGet]
        [ActivityLog(ActivityType = "View", Description = "Opened Master Website Industry List", MenuName = "Website_MasterIndustry")]
        public IActionResult MasterViewWebsiteIndustry() => View();

        [HttpPost]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Get Master Website Industry List", MenuName = "Website_MasterIndustry")]
        public Task<IActionResult> GetWebsiteIndustry()
          => GetPagedDataAsync<Website_MasterIndustryDto, Website_MasterIndustryCommand>(_websiteIndustryService, dto => new Dictionary<string, object>
          {
              ["uuid"] = dto.UUID,
              ["title"] = dto.Title ?? "",
              ["description"] = dto.Description ?? "",
              ["sequence"] = dto.Sequence,
              ["image"] = !string.IsNullOrEmpty(dto.ImageURL)
                  ? $"<img src='{_domainResolverService.BuildAbsoluteUrl(dto.ImageURL)}' alt='Website Industry Image' style='width: 50px; height: 50px; object-fit: cover; border-radius: 4px;' />"
                  : "", // ADDED
              ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleWebsiteIndustry), "website_industry")
          });

        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Website Industry", MenuName = "Website_MasterIndustry")]
        public IActionResult MasterAddWebsiteIndustry()
        {
            var vm = new WebsiteMasterIndustryVM
            {
                MasterInd = new Website_MasterIndustryCommand
                {
                    IsActive = true
                },
                IndPoints = new Website_IndustryPointsCommand(),
                IndCases = new Website_IndustryCasesCommand()
            };
        ...
        [HttpGet]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Edit Master Website Industry", MenuName = "Website_MasterIndustry")]
        public async Task<IActionResult> MasterEditWebsiteIndustry(string? uuid)
        {
            if (string.IsNullOrWhiteSpace(uuid))
                return RedirectToAction(nameof(MasterViewWebsiteIndustry));

            var dto = await _websiteIndustryService.GetByUuidAsync(uuid);
            ...
            var vm = new WebsiteMasterIndustryVM
            {
                MasterInd = new Website_MasterIndustryCommand
                {
                    ...
                    Title = dto.Title,
                    Description = dto.Description,
                    ImageURL = _domainResolverService.BuildAbsoluteUrl( dto.ImageURL),
                    Sequence = dto.Sequence,
            ...
            return View("MasterAddWebsiteIndustry", vm);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        [ActivityLog(ActivityType = "Click", Description = "Clicked On Add Master Website Industry", MenuName = "Website_MasterIndustry")]
        public async Task<IActionResult> MasterAddWebsiteIndustry(WebsiteMasterIndustryVM vm)
        {
            var command = vm.MasterInd;

            if (command.ImageURL == null && command.WebIndustryImageFile == null)
            {
                ModelState.AddModelError(
                    "MasterInd.WebIndustryImageFile",
                    "Required!");
            }

            ModelState.Remove("IndPoints.Title");
            ModelState.Remove("IndPoints.IndustryUUID");
            ModelState.Remove("IndCases.Title");
            ModelState.Remove("IndCases.Icon");
            ModelState.Remove("IndCases.IndustryUUID");
            ...
            try
            {
                ...
                if (command.WebIndustryImageFile != null)
                {
                    command.ImageURL =
                        await _fileUploadService.SaveFileAsync(
                            command.WebIndustryImageFile,
                            await GetCompanyNameAsync(),
                            "website-industry",
                            allowedExtensions);

                    command.ImageUrlwithdomain =
                        _domainResolverService.BuildAbsoluteUrl(
                            command.ImageURL);
                }

                await _websiteIndustryService.SaveAsync(
                    command,
                    GetUserUUID(),
                    Utils.GetLocalIPAddress());

                SetSuccessMessage(
                    string.IsNullOrEmpty(command.UUID)
                        ? "Website Industry added successfully!"
                        : "Website Industry updated successfully!");

                return RedirectToAction(
                    nameof(MasterEditWebsiteIndustry),
        ...
        [HttpPost]
        [ActivityLog(ActivityType = "Update", Description = "Toggled Master Website Industry Status", MenuName = "Website_MasterIndustry")]
        public Task<IActionResult> ToggleWebsiteIndustry(string uuid)
            => ToggleActiveAsync<Website_MasterIndustryDto, Website_MasterIndustryCommand>(uuid, _websiteIndustryService, "Website Industry");


        [HttpPost]
        public async Task<IActionResult> AddIndustryPoint(WebsiteMasterIndustryVM vm)
        {
            try
            {
                ...
                ModelState.Remove("IndCases.Icon");
                ModelState.Remove("IndCases.IndustryUUID");
                ModelState.Remove("MasterInd.Title");
                ModelState.Remove("MasterInd.Description");
                ...
                await _websitepointsService.SaveAsync(
                    vm.IndPoints,
                    GetUserUUID(),
                    Utils.GetLocalIPAddress());
                ...
                {
                    success = true,
                    uuid = vm.IndPoints.UUID,
                    message = "Industry Point added successfully."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> AddIndustryCase(WebsiteMasterIndustryVM vm)
        {
            try
            {

                ModelState.Remove("IndPoints.Title");
                ModelState.Remove("IndPoints.Icon");
                ModelState.Remove("IndPoints.IndustryUUID");
                ModelState.Remove("MasterInd.Title");
                ModelState.Remove("MasterInd.Description");
                ...
                await _websitecasesService.SaveAsync(
                    vm.IndCases,
                    GetUserUUID(),
                    Utils.GetLocalIPAddress());
                ...
                {
                    success = true,
                    uuid = vm.IndCases.UUID,
                    message = "Industry Case added successfully."
                });
            }
            catch (Exception ex)
            {
        ...
        public Task<IActionResult> ToggleIndustryPoints(string uuid)
            => ToggleActiveAsync<Website_IndustryPointsDto, Website_IndustryPointsCommand>(uuid, _websitepointsService, "Website_IndustryPoints");

        public Task<IActionResult> ToggleIndustryCases(string uuid)
           => ToggleActiveAsync<Website_IndustryCasesDto, Website_IndustryCasesCommand>(uuid, _websitecasesService, "Website_IndustryCases");

        [HttpPost]
        public async Task<IActionResult> GetIndustryPoints(string industryUuid)
        {
            return await GetPagedDataAsync<Website_IndustryPointsDto, Website_IndustryPointsCommand>(
                _websitepointsService,
                dto => new Dictionary<string, object>
                {
                    ["uuid"] = dto.UUID,
                    ["title"] = dto.Title,
                    ["sequence"] = dto.Sequence,
                    ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleIndustryPoints), "industry_point")
                },
                x => x.IndustryUUID == industryUuid
            );
        }


        [HttpPost]
        public async Task<IActionResult> GetIndustryCases(string industryUuid)
        {
            return await GetPagedDataAsync<
                Website_IndustryCasesDto,
                Website_IndustryCasesCommand>(
                _websitecasesService,
                dto => new Dictionary<string, object>
                {
                    ["uuid"] = dto.UUID,
                    ["title"] = dto.Title,
                    ["icon"] = dto.Icon,
                    ["sequence"] = dto.Sequence,
                    ["action"] = GetToggleHtml(dto.UUID, dto.IsActive, nameof(ToggleIndustryCases), "industry_case")
                },
                x => x.IndustryUUID == industryUuid
            );
        }
        ...
        [HttpGet]
        public async Task<IActionResult> GetIndustryPoint(string uuid)
        {
            var data = await _websitepointsService.GetByUuidAsync(uuid);
            return Json(data);
        }

        [HttpGet]
        public async Task<IActionResult> GetIndustryCase(string uuid)
        {
            var data = await _websitecasesService.GetByUuidAsync(uuid);
            return Json(data);
        }
