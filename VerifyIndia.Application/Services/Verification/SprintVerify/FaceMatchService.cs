...
namespace VerifyIndia.Application.Services.Verification.SprintVerify
{
    public class FaceMatchService: IFaceMatchService
    {
        ...
        public FaceMatchService(SprintVerifyClient client, SprintMapper sprintMapper)
        {
            _client = client;
            _sprintMapper = sprintMapper;
        }

        public async Task<ApiResponse<JsonElement>> FaceMatchVerify(FaceMatchDto request)
        {
            try
            {
                var result = await _client.PostAsync(ApiEndpoints.FACE_MATCH, request);
